...
const HomeBusqueda = () => {
  ...
  var producto = useParams().producto
  const categoria=[]
  
  if(producto ==="tecnologia" || producto === "electrodomesticos" || producto === "hogar"
  || producto === "herramientas"|| producto === "moda"|| producto === "juguetes"|| producto === "construccion"){
  ...
  useEffect(()=>{
    dispatch(getAllProducts(1, producto, coloresSeleccionados, categoria, ordenPrecio))
  ...
  if(productos2.length === 0 ){
    return <h2>Ups no se encontraron resultados</h2>
  }
  const arrayCats = () => {
    var array = [];
  ...
  return (
    ...
    >
      ...
      {productos2 && productos2.slice(0, productosMostrados).map((prods) => (
        <Grid item xs={12} sm={6} md={6} lg={6} key={prods.id}>
        {prods && prods.fotoprinc && (
